...
remove_chars = re.compile(r"[^a-z]")
remove_spaces = re.compile(r" {2,}")
endings = re.compile(r".{3,}ing$|.{3,}ings$|.{3,}ly$|.{3,}ed$|.{2,}[^aeious]s$|ses$")
endings_remove = re.compile(r"ing$|ings$|ly$|ed$|es$|s$")


def stem(token):
    while endings.findall(token):
        token = endings_remove.sub("", token)
    return token
